...
def main():
    ...
    data_dir = r'Y:\Training_Data\GPC'
    logdir = r'D:\wwr\Second\sample\runs_GPC'
    # Setup parameters
    batch_size = 1
    classes = 2
    nchannels = 3
    ...
    output_dir_seg = os.path.join(r'Y:\Training_Data\GPC_pred')
    ...
    with torch.no_grad():
        for img_path, x in tqdm(dataloader):
            ...
            for t in range(x.shape[0]):
                ...
                # tif.imwrite(os.path.join(output_dir_seg, img_path[t].split('\\')[-1][:-4] + '_logits.tif'),
                #             logits[t, 1].cpu().numpy())
                cv2.imwrite(os.path.join(output_dir_seg, img_path[t].split('\\')[-1][:-4] + '.png'),
                            ypred_seg[t].cpu().numpy())
                ypred_seg[ypred_seg == 1] = 255
                cv2.imwrite(os.path.join(output_dir_seg, img_path[t].split('\\')[-1][:-4] + '_c.png'),
                            ypred_seg[t].cpu().numpy())
                ypred_seg = ypred_seg[t].cpu().numpy()
                combine = cv2.addWeighted(img, 0.5, np.array(Image.fromarray(ypred_seg.astype('uint8')).convert('RGB')),
                                          0.5, 0)
                tif.imwrite(os.path.join(output_dir_seg, img_path[t].split('\\')[-1][:-4] + '.tif'), combine)
