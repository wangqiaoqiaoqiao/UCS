...
class SegmentationMetric(nn.Module):
    ...
    def Kappa(self):
        ...
        observed_accuracy = torch.diag(self.confusionMatrix).sum() / total

        # Expected accuracy (p_e)
        row_sum = torch.sum(self.confusionMatrix, dim=1)
        col_sum = torch.sum(self.confusionMatrix, dim=0)
        expected_accuracy = (row_sum * col_sum).sum() / (total ** 2)

        # Kappa coefficient
        kappa = (observed_accuracy - expected_accuracy) / (1 - expected_accuracy + 1e-8)
        return kappa
