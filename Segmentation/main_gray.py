...
from TTT_loader import myImageFloder_IRN_pseudo_Ge_No_Color
...
def main():
    ...
    nchannels = 3
    device = 'cuda'
    logdir = r'.\runs\pos_pseudov2_Gray_0.1'
    global best_acc
    best_acc = 0
    ...
    traindataloader_pos = torch.utils.data.DataLoader(
        myImageFloder_IRN_pseudo_Ge_No_Color(label_dir, trainlist_pos, aug=True, channels=nchannels),
        batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    testdataloader = torch.utils.data.DataLoader(
        # myImageFloder( testlablist, aug=False, channels=nchannels),
        myImageFloder_IRN_pseudo_Ge_No_Color(label_dir, testlist, aug=False, channels=nchannels),
    ...
    # print the model
    start_epoch = 0
    resume = os.path.join(logdir, 'checkpoint19.tar')
    ...
    for epoch in range(epochs-start_epoch):
        ...
        val_loss, val_oa, val_iou = vtest_epoch(net, criterion, testdataloader, device, epoch, classes, logdir)
        # save every epoch
...
def vtest_epoch(model, criterion, dataloader, device, epoch, classes, logdir):
    model.eval()
    acc_total = SegmentationMetric(numClass=classes, device=device)
    losses = AverageMeter()
    num = len(dataloader)
    ...
    oa = acc_total.OverallAccuracy()
    iou = acc_total.IntersectionOverUnion()
    acc2file(acc_total, os.path.join(logdir, 'checkpoint'+str(epoch)+'.txt'))
    print('epoch %d, train oa %.3f, miou: %.3f' % (epoch, oa, acc_total.meanIntersectionOverUnion()))
    return losses.avg, oa, iou
